...
def scrape_and_save_to_csv(product_link, csv_file):
...
def scrapeFlipkartSearch(search_query, max_pages=5):

    base_url = f'https://www.flipkart.com/search?q={search_query}&otracker=search&otracker1=search&marketplace=FLIPKART&as-show=on&as=off&page=2'
    headers = {
    ...
    for page in range(1, max_pages + 1):
        ...
        csv_file = f'{search_query}_page_{page}_product_data.csv'
        
        for product_link in product_links:
            scrape_and_save_to_csv(product_link, csv_file)

search_query = 'car'
scrapeFlipkartSearch(search_query, max_pages=5)

...
def scrape_flipkart_page(url, product_data_list, lock):
...
def scrape_flipkart_search(max_pages=None, search_query='car'):
    ...
    manager = Manager()
    product_data_list = manager.list()
    lock = manager.Lock()
    ...
    while page <= max_pages:
        ...
        if links:
            ...
            for product_link in product_links:
                proc = Process(target=scrape_flipkart_page, args=(product_link, product_data_list, lock))
                proc.start()
                procs.append(proc)
